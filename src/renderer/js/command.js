...
const CommandBar = {
  ...
  commands: [
    ...
    { id: 'toolbox', label: 'Toolbox', hint: 'Your built-in tools — regex, JSON, hashes, color, word count, and more', icon: '🧰', action: () => { try { if (window.Toolbox) Toolbox.open(); } catch {} } },
    { id: 'jobsetup', label: 'Personalize for Your Job', hint: 'Pick your profession — Vex applies a fitting theme + the tools you use daily', icon: '🧑‍💼', action: () => { try { if (window.JobSetup) JobSetup.open(); } catch {} } },
    ...
    { id: 'siteidentity', label: "Show This Site's Browser Identity", hint: 'What this page sees — user-agent, Chrome brand, window.chrome, webdriver, WebGL — with a PASS/FAIL compatibility verdict that diagnoses “unsupported browser” gates', icon: '🕵️', action: () => { try { if (typeof SiteIdentity !== 'undefined') SiteIdentity.open(); } catch {} } },
