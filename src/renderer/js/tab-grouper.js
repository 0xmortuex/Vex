  ...
  async function analyzeAndPropose(onlyUngrouped = true) {
    ...
    try {
      ...
      const tabMeta = tabsToAnalyze.map(t => {
        ...
        // Keys MUST match the AI contract: both the cloud worker
        // (vex-ai-worker/worker.js group-tabs handler reads t.title/t.url/
        // t.summary) and the local Ollama prompt (ai-router.js groupTabs:
        // "Given tabs (id, title, url, summary)") expect these exact names.
        // Sending abbreviated t/u/s left every title/url blank, so the model
        // had nothing to cluster on and returned no usable groups — the
        // "Organize My Tabs does nothing" bug. url carries the bare hostname
        // (via _domain) intentionally: better clustering signal than full URLs.
        return {
          id: t.id,
          title: (t.title || '').substring(0, 60),
          url: _domain(t.url),
          summary: (h && h.summary) ? String(h.summary).substring(0, 100) : ''
        };
      });
