...
(async function () {
  ...
  // Default (and fallback for any unknown/legacy value) is Oxford Editorial.
  ...
  // tabs on top of the already-restored set, doubling them — it has been removed.

  // Workspace switcher button
  document.getElementById('workspace-btn')?.addEventListener('click', () => WorkspaceManager.toggleDropdown());

  // PiP lives in js/pip-manager.js (loaded before this file) so its pop-out
  // behaviour can be tested — the control bar is in a closed shadow root.
  window.PiPManager.init();

  // === URL Bar ===
  const urlInput = document.getElementById('url-input');

  // Shared navigate logic: resolve a typed string to a URL (or search), then
  // navigate the active tab (or open a new tab with newTab=true). Extracted so
  // the Smart Searchbar suggestion path reuses the EXACT same submit behavior.
  function submitUrlBar(val, newTab) {
    val = (val || '').trim();
    if (!val) return;
  ...
  // Smart Searchbar: local-only suggestion dropdown (open tabs, history,
  // bookmarks). Selecting a suggestion routes through submitUrlBar so it
  // behaves identically to a typed navigation.
  if (typeof SmartSearchbar !== 'undefined') {
    SmartSearchbar.init(urlInput, { onSubmit: (url, newTab) => submitUrlBar(url, newTab) });
  }

  urlInput.addEventListener('keydown', (e) => {
    // Let the Smart Searchbar consume arrow-nav / Enter-on-highlight / Esc-
    // while-open FIRST. It returns true only when it handled the key; on false
    // (e.g. typed Enter with nothing highlighted, or dropdown closed) we run
    // the normal logic below unchanged — zero regression to type-and-Enter.
    if (typeof SmartSearchbar !== 'undefined' && SmartSearchbar.handleKeydown(e)) {
      return;
    }

    if (e.key === 'Enter') {
      submitUrlBar(urlInput.value, e.ctrlKey || e.altKey);
  ...
  // When the user leaves the bar, re-sync it to the real active-tab URL.
  // updateUrlBar() skips overwriting while the bar is focused (so async webview
  // load events can't wipe an in-progress paste/type), which means the bar may
  // hold stale edit text at blur time. Re-syncing here guarantees the displayed
  // value always matches the actual page once editing ends. Enter/Escape blur
  // the bar first, so they flow through this same path too.
  ...
  // Personalization + self-hosted cloud config. All optional; stored in
  ...
  (function wirePersonalizationSettings() {
    ...
    const refreshStatus = () => {
      if (!status) return;
      const ai = (() => { try { return (localStorage.getItem('vex.aiWorkerUrl') || '').trim(); } catch { return ''; } })();
  ...
  if (typeof ShortcutsRegistry !== 'undefined') {
    ...
    ShortcutsRegistry.register('toggle-theme',   () => {
      ...
      if (typeof ThemePicker !== 'undefined') { ThemePicker.toggle(); return; }
  ...
  // Save workspace state before window closes. (Open tabs are persisted
  // continuously by TabManager — no separate session snapshot needed.)
