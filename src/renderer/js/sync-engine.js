...
const SyncEngine = (() => {
  ...
  async function requestCode(email) {
    requireSyncUrl();
    const r = await fetch(`${syncWorkerUrl()}/auth/request-code`, {
      method: 'POST',
  ...
  async function verifyCode(email, code, deviceName) {
    requireSyncUrl();
    const r = await fetch(`${syncWorkerUrl()}/auth/verify-code`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function pushNow() {
    ...
    try {
      ...
      const r = await fetch(`${syncWorkerUrl()}/sync/push`, {
        method: 'POST',
        headers: {
  ...
  async function pullNow() {
    ...
    state.syncing = true;
    try {
      const r = await fetch(`${syncWorkerUrl()}/sync/pull`, {
        headers: { 'Authorization': `Bearer ${state.sessionToken}` }
      });
      if (r.status === 401) { await signOut(); return { ok: false, reason: 'unauthorized' }; }
