...
### Added
- **Auto-fill email verification codes from your open Gmail.** When a site asks for a code it emailed you, and you have Gmail (web) open in a Vex tab, Vex reads the newest code from your inbox and fills it into the code field — no switching tabs, opening the email, and copying. It reads *only* a Gmail tab you already have open (no stored credentials, no email backend), prefers a code from an unread email near verification wording, fills only a real one-time-code field, and retries for a bit since the email usually lands a few seconds after you ask for it. *(Keep Gmail open and awake for it to work; app-based 2FA still uses the Authenticator.)*
