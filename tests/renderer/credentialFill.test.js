...
it('rejects TOTP after a navigation while reading codes', async () => {
  ...
  // Looking for the field is a read-only DOM probe and happens before the
  // navigation; what must not happen is the code reaching the new page.
...
it('matches an authenticator entry that has no issuer at all', async () => {
  // main.js stores issuer:'' when a secret is typed in by hand rather than
  // scanned, so keying only on issuer meant those entries never autofilled.
  window.vex = { totpList: async () => [{ id: 'r', issuer: '', label: 'Roblox' }], totpCodes: async () => [{ id: 'r', code: '112233' }] };
  window.vexConfirm = vi.fn(async () => true);
  const { webview } = page('<input autocomplete="one-time-code">', 'https://www.roblox.com/login');
  await TotpAutofill.autofill(webview, webview.getURL());
  expect(document.querySelector('input').value).toBe('112233');
});
it('matches an issuer carrying a company suffix', async () => {
  window.vex = { totpList: async () => [{ id: 'r', issuer: 'Roblox Corporation', label: 'me' }], totpCodes: async () => [{ id: 'r', code: '445566' }] };
  window.vexConfirm = vi.fn(async () => true);
  const { webview } = page('<input autocomplete="one-time-code">', 'https://www.roblox.com/login');
  await TotpAutofill.autofill(webview, webview.getURL());
  expect(document.querySelector('input').value).toBe('445566');
});
it('matches an "Issuer:account" label with no separate issuer', async () => {
  window.vex = { totpList: async () => [{ id: 'r', issuer: '', label: 'Roblox: myname' }], totpCodes: async () => [{ id: 'r', code: '778899' }] };
  window.vexConfirm = vi.fn(async () => true);
  const { webview } = page('<input autocomplete="one-time-code">', 'https://www.roblox.com/login');
  await TotpAutofill.autofill(webview, webview.getURL());
  expect(document.querySelector('input').value).toBe('778899');
});
it('fills a field labelled only "Code" when the page is clearly a 2FA screen', async () => {
  // Roblox labels its input just "Code". Refusing a bare "code" everywhere meant
  // 2FA autofill never fired there; accepting it anywhere would grab promo boxes.
  window.vex = { totpList: async () => [{ id: 'r', issuer: 'Roblox' }], totpCodes: async () => [{ id: 'r', code: '314159' }] };
  window.vexConfirm = vi.fn(async () => true);
  const { webview } = page('<p>Enter the code from your authenticator app</p><input placeholder="Code">', 'https://www.roblox.com/login');
  await TotpAutofill.autofill(webview, webview.getURL());
  expect(document.querySelector('input').value).toBe('314159');
});
it('fills the real Roblox 2-Step Verification screen', async () => {
  // Markup taken from the actual page. Its input is labelled only
  // "Enter 6-digit Code" — no autocomplete=one-time-code, no "authenticator" or
  // "verification code" wording on the field itself — so the old detector never
  // recognised it and the confirmation prompt never even appeared.
  window.vex = { totpList: async () => [{ id: 'r', issuer: 'Roblox', label: 'Roblox' }], totpCodes: async () => [{ id: 'r', code: '123456' }] };
  window.vexConfirm = vi.fn(async () => true);
  const { webview } = page(
    '<h2>2-Step Verification</h2><p>Enter the code generated by your authenticator app.</p>'
    + '<input placeholder="Enter 6-digit Code"><button>Verify</button>',
    'https://www.roblox.com/login');
  await TotpAutofill.autofill(webview, webview.getURL());
  expect(window.vexConfirm).toHaveBeenCalledTimes(1);
  expect(document.querySelector('input').value).toBe('123456');
});
it('does not fill a bare "code" box on a page that is not a 2FA screen', async () => {
  window.vex = { totpList: async () => [{ id: 'r', issuer: 'Roblox' }], totpCodes: vi.fn() };
  window.vexConfirm = vi.fn(async () => true);
  const { webview } = page('<p>Have a promo code? Redeem it below.</p><input placeholder="Code">', 'https://www.roblox.com/redeem');
  await TotpAutofill.autofill(webview, webview.getURL());
  expect(window.vexConfirm).not.toHaveBeenCalled();
  expect(document.querySelector('input').value).toBe('');
});
...
it('waits for a 2FA screen that appears after the password step', async () => {
  // Roblox shows 2-Step Verification as a modal on the SAME url, so dom-ready
  // never fires again. Running once at page load and giving up meant the field
  // did not exist yet at the only moment we ever looked.
  vi.useFakeTimers();
  window.vex = { totpList: async () => [{ id: 'r', issuer: 'Roblox' }], totpCodes: async () => [{ id: 'r', code: '867530' }] };
  window.vexConfirm = vi.fn(async () => true);
  const { webview } = page('<input type="password">', 'https://www.roblox.com/login');
  const pending = TotpAutofill.autofill(webview, webview.getURL());
  await vi.advanceTimersByTimeAsync(3000);
  // The 2FA modal arrives a few seconds later.
  document.body.innerHTML = '<h2>2-Step Verification</h2><p>Enter the code generated by your authenticator app.</p><input placeholder="Enter 6-digit Code">';
  document.querySelector('input').getBoundingClientRect = () => ({ width: 100, height: 20 });
  await vi.advanceTimersByTimeAsync(6000);
  await pending;
  expect(document.querySelector('input').value).toBe('867530');
});
it('does not sit polling on an ordinary page of a site with a saved code', async () => {
  vi.useFakeTimers();
  window.vex = { totpList: async () => [{ id: 'r', issuer: 'Roblox' }], totpCodes: vi.fn() };
  const { webview } = page('<input type="search" placeholder="Search">', 'https://www.roblox.com/home');
