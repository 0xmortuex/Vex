...
- **What this audit did NOT cover:** dependency CVEs (`npm audit` reports 27 vulnerabilities — out of scope here, file separately), runtime behaviour, fuzzing, the Cloudflare AI worker (server-side), the auto-update channel, code-signing posture, or the build pipeline.
...
- **M-3 geolocation bridge over-exposed:** ✅ fixed in `src/preload-webview.js` — surface restricted to a single atomic `resolveLocation(origin)` method (was: `checkPermission` + `getPref`, where `getPref` was callable without permission). The new method does permission-check + pref-read + coord-coarsening in one round-trip and returns one of `{mode: 'denied'}` / `{mode: 'manual', latitude, longitude}` / `{mode: 'ip'}`. Coords are rounded to 1 decimal place (~11 km, city-level) by `coarsenLocation()` in `src/main-helpers.js` before they cross into the renderer. Any extra fields on the raw pref (ISP, ASN, IP, timezone, accuracy, timestamp) are stripped. Audit's Option B (randomised bridge name for fingerprint mitigation) left as a `TODO(security)` follow-up since the surface tightening was the higher-priority half. Tests in `tests/main/geoBridge.test.js`.

Shared helpers `safeJoin`, `safeName`, `safePipUrl`, and `coarsenLocation` live in `src/main-helpers.js`. Tests in `tests/main/safePath.test.js` and `tests/main/geoBridge.test.js`. Full suite: **200 passing, 0 failed**.
